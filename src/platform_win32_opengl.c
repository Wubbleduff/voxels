    ...
    const mtx4x4* mvp)
{
    ...
    CALL_GL(glUniformMatrix4fv, loc, 1, 1, &mvp->m[0]);
...
void do_one_frame(struct GameState* prev_game_state, struct GameState* next_game_state);
...
void WinMainCRTStartup()
{
    ...
    ASSERT(game_memory_storage, "Could not allocate game memory.");
    struct GameMemory* game_memory = (struct GameMemory*)game_memory_storage;

    g_platform = &game_memory->platform_state;
    struct Platform_Win32* platform = g_platform;

    // Init logging file.
    platform->log_file = CreateFileA("log.txt", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ASSERT(platform->log_file != INVALID_HANDLE_VALUE, "Couldn't open log file for writing.");
    ...
    // Init engine graphics state.
    {
        ...
        opengl_state->glDrawElements = (fnptr_glDrawElements)load_gl_fn(opengl32_dll_module, "glDrawElements");
    ...
    struct GameState* game_state_A = &game_memory->game_state_A;
    struct GameState* game_state_B = &game_memory->game_state_B;
